            ...
            <form method="get" action="" class="shb-filter-inline">
                ...
                <select name="status">
                    ...
                    <option value="pending" <?php selected($status_filter, 'pending'); ?>>
                        <?php _e('Pending', 'sanctuary-hotel-booking'); ?></option>
                    <option value="confirmed" <?php selected($status_filter, 'confirmed'); ?>>
                        <?php _e('Confirmed', 'sanctuary-hotel-booking'); ?></option>
                    <option value="checked_in" <?php selected($status_filter, 'checked_in'); ?>>
                        <?php _e('Checked In', 'sanctuary-hotel-booking'); ?></option>
                    <option value="checked_out" <?php selected($status_filter, 'checked_out'); ?>>
                        <?php _e('Checked Out', 'sanctuary-hotel-booking'); ?></option>
                    <option value="cancelled" <?php selected($status_filter, 'cancelled'); ?>>
                        <?php _e('Cancelled', 'sanctuary-hotel-booking'); ?></option>
                </select>
    ...
    <?php if ($view === 'calendar'): ?>
        ...
        <div class="shb-calendar-container" id="shb-calendar-container">
            <div class="shb-calendar-controls">
                <div class="shb-calendar-nav">
                    <button type="button" class="button" id="shb-cal-prev">&laquo;
                        <?php _e('Prev', 'sanctuary-hotel-booking'); ?></button>
                    <h2 id="shb-cal-title" style="margin:0 16px;font-size:18px"></h2>
                    <button type="button" class="button" id="shb-cal-next"><?php _e('Next', 'sanctuary-hotel-booking'); ?>
                        &raquo;</button>
                </div>
            ...
            <div class="shb-calendar-legend" id="shb-calendar-legend">
                <?php foreach ($rooms as $room): ?>
                    <span class="shb-legend-item" data-room-id="<?php echo esc_attr($room['id']); ?>">
                        <span class="shb-legend-dot"
                            style="background:<?php echo esc_attr($room_colors[$room['id']]); ?>"></span>
                        <?php echo esc_html($room['name']); ?>
                    </span>
                <?php endforeach; ?>
                <span class="shb-legend-item shb-legend-blocked">
                    <span class="shb-legend-dot shb-blocked-pattern"></span>
                    <?php _e('Blocked/Unavailable', 'sanctuary-hotel-booking'); ?>
                </span>
            </div>
        ...
        <script>
            jQuery(document).ready(function ($) {
                ...
                // Availability blocks from Availability settings
                var allBlocks = <?php echo json_encode(array_map(function ($block) use ($rooms) {
                    $room_name = 'Unknown';
                    foreach ($rooms as $room) {
                        if ($room['id'] == $block['room_id']) {
                            $room_name = $room['name'];
                            break;
                        }
                    }
                ...
                function renderCalendar(year, month, roomFilter) {
                    ...
                    // Filter availability blocks
                    var filteredBlocks = allBlocks;
                    if (roomFilter && roomFilter !== 'all') {
                        filteredBlocks = allBlocks.filter(function (block) { return block.room_id == roomFilter; });
                    }
                    ...
                    if (roomFilter && roomFilter !== 'all') {
                        ...
                        $('.shb-legend-blocked').show(); // Always show blocked legend
                    } else {
                    ...
                    for (var day = 1; day <= daysInMonth; day++) {
                        ...
                        // Check for availability blocks on this day
                        var dayBlocks = filteredBlocks.filter(function (block) {
                            return dateStr >= block.start_date && dateStr <= block.end_date;
                        });
                        var isBlocked = dayBlocks.length > 0;

                        html += '<div class="shb-cal-cell' + (isToday ? ' shb-cal-today' : '') + (isBlocked ? ' shb-cal-blocked' : '') + '" data-date="' + dateStr + '">';
                        html += '<span class="shb-cal-date">' + day + '</span>';
                        
                        // Show blocked indicator
                        if (isBlocked) {
                            var blockInfo = dayBlocks.map(function(b) { 
                                return b.room_name + ' (' + b.reason + ')'; 
                            }).join(', ');
                            html += '<div class="shb-cal-events">';
                            html += '<div class="shb-cal-event shb-cal-event-blocked" title="Blocked: ' + blockInfo + '">';
                            html += '<span class="dashicons dashicons-lock" style="font-size:12px;width:12px;height:12px;"></span>';
                            html += '</div>';
                            html += '</div>';
                        }
                        ...
                        // Close events div if we opened it for blocked
                        if (isBlocked && dayBookings.length > 0) {
                            html += '</div>';
                        }
                ...
                <tbody>
                    <?php foreach ($bookings as $booking):
                        ...
                        <tr data-booking-id="<?php echo esc_attr($booking['id']); ?>">
                            <td><span
                                    style="display:inline-block;width:12px;height:12px;border-radius:3px;background:<?php echo esc_attr($bcolor); ?>"></span>
                            </td>
                            ...
                            <td><?php echo esc_html(get_option('shb_currency_symbol', '$') . number_format($booking['total_price'], 2)); ?>
                            </td>
                            <td>
                            ...
                            <td>
                                <select class="shb-status-select" data-booking-id="<?php echo esc_attr($booking['id']); ?>">
                                    <option value="pending" <?php selected($booking['booking_status'], 'pending'); ?>>
                                        <?php _e('Pending', 'sanctuary-hotel-booking'); ?></option>
                                    <option value="confirmed" <?php selected($booking['booking_status'], 'confirmed'); ?>>
                                        <?php _e('Confirmed', 'sanctuary-hotel-booking'); ?></option>
                                    <option value="checked_in" <?php selected($booking['booking_status'], 'checked_in'); ?>>
                                        <?php _e('Checked In', 'sanctuary-hotel-booking'); ?></option>
                                    <option value="checked_out" <?php selected($booking['booking_status'], 'checked_out'); ?>>
                                        <?php _e('Checked Out', 'sanctuary-hotel-booking'); ?></option>
                                    <option value="cancelled" <?php selected($booking['booking_status'], 'cancelled'); ?>>
                                        <?php _e('Cancelled', 'sanctuary-hotel-booking'); ?></option>
                                </select>
                            </td>
                            <td>
                                <div class="shb-action-group">
                                    ...
                                    <a href="<?php echo esc_url(add_query_arg('shb_download_calendar', $booking['id'], home_url())); ?>"
                                        ...
                                        title="<?php _e('Download Calendar', 'sanctuary-hotel-booking'); ?>">
