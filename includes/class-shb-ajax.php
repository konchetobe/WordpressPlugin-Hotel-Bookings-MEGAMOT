...
class SHB_Ajax
{
    ...
    public static function create_booking()
    {
        ...
        if ($payment_method === 'stripe') {
            // For Stripe/card payment, create payment session and redirect to checkout
            // Booking will NOT be confirmed until payment is successful
            $payment_result = SHB_Payments::create_stripe_checkout($result['booking_id']);

            if (is_wp_error($payment_result)) {
                // Payment session creation failed - return error
                wp_send_json_error(array('message' => $payment_result->get_error_message()));
            }

            // Return redirect URL to Stripe checkout - user MUST complete payment
            ...
            // Return bank details for the user to make the transfer
            $bank_details = array(
                'account_holder' => get_option('shb_bank_account_holder', ''),
                'iban' => get_option('shb_bank_iban', ''),
                'bic' => get_option('shb_bank_bic', ''),
                'bank_name' => get_option('shb_bank_name', ''),
                'instructions' => get_option('shb_bank_instructions', __('Please use your booking reference as the payment reference. Your booking will be confirmed once we receive the payment.', 'sanctuary-hotel-booking')),
            );
        ...
        // For other non-online payments (future: pay at hotel, etc.), go to confirmation
    ...
    public static function admin_save_pricing_rule()
    {
        ...
        $data = array(
            ...
            'location_id' => absint($_POST['location_id'] ?? 0),
            'room_id' => absint($_POST['room_id'] ?? 0),
            'room_type_term_id' => absint($_POST['room_type_term_id'] ?? 0),
